...
def _lsp_records(caplog_lsp):
    return [r for r in caplog_lsp.records if r.name == "hermes.lint.lsp"]
...
def test_clean_emits_at_debug(caplog_lsp):
    ...
    info_records = [r for r in _lsp_records(caplog_lsp) if r.levelno >= logging.INFO]
    debug_records = [r for r in _lsp_records(caplog_lsp) if r.levelno == logging.DEBUG]
    assert info_records == []
...
def test_active_for_fires_per_distinct_root(caplog_lsp):
    ...
    info = [r for r in _lsp_records(caplog_lsp) if r.levelno == logging.INFO]
    assert len(info) == 2
...
def test_server_unavailable_warns_once_per_binary(caplog_lsp):
    ...
    warns = [r for r in _lsp_records(caplog_lsp) if r.levelno == logging.WARNING]
    assert len(warns) == 1
